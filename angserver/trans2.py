...
class Trans:
    def __init__(self, data):
        ...
        self.rtnData['unit_no'] = self.data['unit_no']  # 集团号
        ...
        self.rtnData['card_type'] = '0' if self.data['track_2'] == '0'*37 else '1'  # 卡类型  0系统卡  1客户卡  系统卡为系统统一发放，有二磁道验证；客户卡只有卡号，无二磁道
        # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< 定义返回的数据
    ...
    def _get_card_info(self, card_no, shop_no):
        if self.rtnData not in ('0', '1'):
            return None

        if self.rtnData['card_type'] == '0':
            sql = """SELECT * FROM card_info WHERE card_no = %s AND unit_no = (
                        SELECT unit_no FROM shop_info WHERE shop_no = %s);
            """
        elif self.rtnData['card_type'] == '1':
            sql  = """SELECT * FROM card_info WHERE custom_card_no=%s AND unit_no=(
                        SELECT unit_no FROM shop_info WHERE shop_no=%s);
            """
    ...
    def deco_is_my_unit_card(self, func):
        def _deco(self, shop_no, card_no):
            sql = """
            ...
            if self.db.fetch_one(sql, params)['count'] == 0:
                self.rtnData['trans_code'] == trans_code['not_my_card']
                ret = self.rtnData
            else:
                ret = func(shop_no, card_no)
            return ret
        return _deco
    ...
    def _add_trans(self,trans_id=None, reversiable='0', points_rule=0, amount_balance=0,
        ...
        sql = """
            INSERT INTO trans (
                ...
                %s, %s, %s,
                %s,
                (SELECT unit_no FROM card_info WHERE card_no = %s),
                (SELECT unit_no FROM shop_info WHERE shop_no = %s),
                %s
            );
        """
        ...
        params = (
                ...
                trans_id, 
                self.data['card_no'], self.data['shop_no'], self.data['interface']
            )
        self.db.execute(sql, params)
        if self.rtnData['trans_code'] in ('000010', '000020', '000030', '000040', '000050', '000060', '000070', '000080', '000190'):
    ...
    def deposit(self):
        ...
        if not self._card_check():
            ...
            return self.rtnData
        # 检查是否是充值卡  -- 2013.01.22
        if not self.data['card_no'].strip()[7:8] == '0':
