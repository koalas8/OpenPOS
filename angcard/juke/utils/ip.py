...
class IpLocater:
    ...
    def get_addr(self, offset, ip=0):
        ...
        (byte,) = unpack('B', str)
        if byte == 0x01:
            countryOffset = self.get_long_3()
            self.ipdb.seek(countryOffset)
            str = self.ipdb.read(1)
            (b,) = unpack('B', str)
            if b == 0x02:
                countryAddr = self.get_string(self.get_long_3())
                self.ipdb.seek(countryOffset + 4)
            else:
                countryAddr = self.get_string(countryOffset)
            areaAddr = self.get_area_addr()
        elif byte == 0x02:
            countryAddr = self.get_string(self.get_long_3())
            areaAddr = self.get_area_addr(offset + 8)
        else:
            countryAddr = self.get_string(offset + 4)
            areaAddr = self.get_area_addr()
    ...
    def output(self, first, last):
        ...
        for index in range(first, last):
            ...
            print "%s - %s" % (ip, self.get_addr(of1 + (of2 << 16)) )
    ...
    def getIpAddr(self, ip):
        ...
        aoffset = self.get_long_3(ioffset + 4)
        address = self.get_addr(aoffset)
        return address
...
if __name__ == "__main__":
    ...
    print "the ip %s come from %s" % (ip, address.split('/')[0])
